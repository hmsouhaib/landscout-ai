...
- Attribute-profile validation first proves strict immutable nested records and aggregate/hash closure, then independently rebuilds from current package bytes. A coordinated caller profile/hash mutation cannot override the physical source, a persistent mutation fails final extraction/catalog validation, and a temporary live-path swap cannot alter the captured package snapshot.
