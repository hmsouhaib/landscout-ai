...
Validated public returns retain no DataFrame, GeoDataFrame, Series, NumPy array, absolute filesystem `Path`, geometry, mutable mapping or mutable sequence. Each layer retains the exact portable source-package `relative_path: str`. The frozen dataclass constructors themselves do not validate arguments; exact nested runtime checks at public boundaries remain necessary.

All fields are required. Optional annotations permit only explicit null where stated, not omitted constructor arguments. `InpnProtectedAreasLayerAttributeProfile` has `relative_path: str`, `file_size: int`, `file_sha256: str`, `package_position: int`, `driver_name: str`, `layer_name: str`, `layer_position: int`, `feature_count: int`, `fid_count: int`, `fid_min: int | None`, `fid_max: int | None`, `fid_sequence_sha256: str`, `row_content_sha256: str`, and `fields: tuple[InpnProtectedAreasFieldAttributeProfile, ...]`. Package sizes are positive, positions/counts are non-negative, driver is exact `GPKG`, and extrema are null exactly for empty layers. FIDs may be sparse or negative; sorting does not renumber them.

`InpnProtectedAreasAttributeProfile` has required exact integer `attribute_profile_schema_version` (1), text `provider`, `authority`, `program`, `dataset_id`, `dataset_name`, `declared_version`, `reference_page_url`, `archive_url`, `archive_filename`, positive integer `archive_size`, text `archive_sha256`, integer `source_catalog_schema_version` (2), text `source_catalog_content_sha256`, `layers: tuple[InpnProtectedAreasLayerAttributeProfile, ...]`, six exact non-negative integer aggregates (`package_count`, `layer_count`, `field_definition_count`, `total_row_count`, `total_null_count`, `total_distinct_non_null_value_count`), and text `complete_attribute_profile_content_sha256`. Digests are lowercase 64-hex strings. `total_null_count` sums null cells across fields; `total_distinct_non_null_value_count` sums each field's domain size, not globally deduplicated values. Layers are nonempty as an inventory, although an individual layer may have no rows or no fields. Field `distinct_values` has exact type `tuple[InpnProtectedAreasDistinctAttributeValue, ...]`; empty text and empty bytes are supported non-null values. `_CanonicalCell.value_kind` and `.canonical_value` are both `str | None`, with `(None, None)` used internally for null.
...
| `_is_null_scalar` | `def _is_null_scalar(value: object) -> bool` | Returns true for None, pd.NA, pd.NaT, Python/NumPy floating NaN and NumPy datetime64 NaT. Other objects, including arrays, return false; unsupported/composite rejection belongs to `_canonical_cell`, not this predicate. |
| `_canonical_cell` | `def _canonical_cell( value: object, *, relative_path: str, layer_name: str, field_name: str, fid: int, ) -> _CanonicalCell` | Checks null first, then exact Python str/bool/int/float/bytes and explicit NumPy str_/bool_/integer/floating families, producing built-in canonical strings. Boolean precedes integer; finite float hex preserves signed zero. Non-null temporal objects, composites, bytearray, Shapely cells and arbitrary unsupported objects fail with package/layer/field/FID context. This is not blanket rejection of every subclass: the NumPy families deliberately use isinstance. |
...
| `_cell_payload` | `def _cell_payload(cell: _CanonicalCell) -> object` | Returns JSON null (`None`) for a null cell, otherwise the two-member list `[value_kind, canonical_value]`. It does not validate arbitrary private cells. |
...
`_prepare_inputs` intentionally performs extraction validation, public catalog validation (which itself rebuilds), and an additional fresh public catalog build before attribute reads. `_build_with_postconditions` builds/intrinsically checks the profile, then validates extraction and rebuilds catalog again. These are read-only authority checks, not downloads or extraction rebuilds. Public validation adds the cheap supplied-profile/catalog comparison before repeating attribute reads; its return value is `None`. Stored non-empty component hashes cannot be recomputed from marginal domains alone, which is why intrinsic validity never substitutes for the physical rebuild.

For each layer, `_profile_layer` addresses cells by the original row position paired with numerically sorted FID, preserving the incoming frame unchanged. It creates complete per-field frequency maps and full FID-addressed row/column payloads in memory; no sampling, top-N truncation, streaming-output file or cross-field co-occurrence table is returned. Kind/value pairs are sorted lexically. Canonical distinct-value validation round-trips FLOAT_HEX and strict Base64 spellings, enforces canonical decimal integer grammar and exact Boolean spelling, and leaves TEXT unchanged. Empty-layer hashes can be independently reconstructed from empty payloads; populated component hashes require physical values.
