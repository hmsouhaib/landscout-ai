...
Standard GeoPackageBinary is not embedded WKB, and neither is the parser-derived canonical WKB. The full source BLOB has its own `GP` header, signed SRS ID, flags, optional envelope, and embedded WKB with independent byte order. The parser validates header structure, the metadata/header SRS relationship, empty-flag agreement, and Z/M declarations. XY, XYZ, XYM, and XYZM—including measured empty geometries—remain distinct. NULL is never EMPTY. Non-empty present ordinates must all be finite; invalid but parseable topology is counted with its exact Shapely validity reason, never repaired. Observed XY bounds are compared exactly with catalog bounds as `EXACT_MATCH`, `DIFFERENT`, or `BOTH_NULL`, without tolerance or CRS inference.
