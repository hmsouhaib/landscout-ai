...
- Geometry metadata accepts only exact built-in uppercase core declarations: `GEOMETRY`, `POINT`, `LINESTRING`, `POLYGON`, `MULTIPOINT`, `MULTILINESTRING`, `MULTIPOLYGON`, and `GEOMETRYCOLLECTION`. The physical geometry-column SQL declared type must exactly equal `gpkg_geometry_columns.geometry_type_name`. Separately, the raw ISO WKB root must be assignable to the declaration: any supported core root for `GEOMETRY`, otherwise only the matching concrete Shapely family. There is no casefold equivalence, coercion, extended-type support, or dimension-based type substitution. NULL has no root, while EMPTY retains its family. Intrinsic validation repeats the declaration/observed-domain relationship, rejecting contradictions even after a coherent complete-profile rehash.
