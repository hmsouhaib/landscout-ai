...
- Records the clean fetched starting SHA, one immutable source-bound evidence bundle, ten-field layer alignment, five new public exports, and unchanged acquisition/catalog/attribute/geometry implementations, suites, config, schemas and reader/hash contracts.
- Separates private canonical structure from physical source trust: both existing public profile validators run once per complete build outside layer loops, the public bundle validator physically reconstructs and compares, and final public extraction validation rejects persistent post-alignment archive/package mutation. Exact configuration reconstruction and chained lower causes remain mandatory.
- Binds canonical package/layer order and full keys, repeated package evidence, source/archive/catalog identity, count/extrema/FID-sequence equality, and the inspected same sorted integer-JSON commitment. Equal-count/equal-extrema `[1, 2, 4]` versus `[1, 3, 4]` rejects; negative/sparse/empty and reversed read-order controls pass.
- Records 25 test functions / 141 new cases, immediate scalar-type failures, same-class forged config rejection, coordinated physically rejected catalog/profile/bundle forgeries, genuine M/ZM ordinates, immutable portable results and honestly labeled pure controls. Completed focused results are 803 existing, 141 new and 944 combined; full result is 3,939 (`3,798 + 141`) with four individually identified pre-existing UserWarnings, no skips/xfails and all exit codes zero.
- Records both observed FID hashes and equality for every physical package/layer: 15 packages, 15 aligned pairs and 11,381 FIDs, zero missing/extra/mismatched layers. New schema-1 bundle SHA is `a558236c6a246999fca83ac10c443cd686807675327cdab2da5405143db56016`; repeat construction, independent physical validation and compact-payload rehash agree. All three approved upstream hashes remain unchanged; the entire geometry payload matches the historical same-toolchain payload, and no unavailable historical attribute JSON comparison is claimed.
- Binds actual phase timings and reader counts: 60 approved attribute-only reads / 45,524 rows; 60 SQLite snapshots and exact FID/BLOB SELECTs / 45,524 rows; 375 calls each to catalog list_layers/read_info. Network/download/lossy-geometry-reader/transformation/reprojection/parcel-analysis sentinels remain zero. Cache hits are offline, tooling is unchanged, and sig_tadl remains unresolved.
- Records all completed quality gates and documentation/source bindings. Only source/profile validation and FID alignment exist: no joined feature output, category/legal/CRS interpretation, geometry correction, parcel policy, exclusion or score. This companion follows the final log edit, with exact heading inventory and complete source snapshot; historical entries remain unchanged.

## STEP 7F.1B.3.2 evidence delta

- Records that STEP 7F.1B.3.1 correctly implemented its review-authored ticket, and this correction restores the GeoPackage 1.4 Requirement 32 / normative Annex G collection-subtype hierarchy that the preceding ticket restricted incorrectly.
- Distinguishes exact declaration vocabulary, exact SQL column/declaration equality, and one private deeply immutable stored-root acceptance relation shared by raw parsing and intrinsic validation. GEOMETRYCOLLECTION accepts GeometryCollection and the three Multi* roots, not bare primitives; Polygon/MultiPolygon remain distinct.
- Binds the independent 56-pair matrix (17 accepts / 39 rejects), exactly three red-first collection/Multi* failures, all 77 new passing cases, and unchanged 327 existing geometry cases. Physical build/independent validation retain actual names and raw bytes; EMPTY XY and genuine non-empty M/ZM tests preserve roots, dimensions and numeric ordinates without redesigning measured-empty collections.
- Records completed exit-zero runs: 399 existing INPN, 404 geometry, 803 combined and 3,798 full-suite cases. The full suite retains four individually identified pre-existing UserWarnings; six new intentional SQL-mismatch RuntimeWarnings and eight existing measured-metadata RuntimeWarnings are explicitly asserted, not unhandled.
- Binds same-toolchain complete real-payload equality and unchanged geometry SHA `997c8c27cbbedb2860778386b3f2eb5afa9f64de6ad07e41fc67b4cec9060ee7`, repeat build and independent validation: 15 packages/layers, 11,381 valid non-empty XY MultiPolygon rows, 10,281,493 coordinates, exact bounds and unchanged unresolved sig_tadl evidence.
- Records unchanged source/schema/encoding/payload/hash/readers/public boundaries, zero forbidden operations, every quality gate, source/test SHA/snapshot audits and the deliberately unread approved attribute profile. Envelope framing remains distinct from numerical envelope-value agreement. Historical entries are preserved, and this companion is synchronized after finalizing the log.

## STEP 7F.1B.3.1 evidence delta

- Records the independent-review gaps against GeoPackage 1.4 requirements 25/31/32: canonical core declaration, exact physical SQL declared type, and assignable actual WKB root, with the same intrinsic declaration/domain relation after rehash.
- Binds the single private immutable core mapping, declaration-only GEOMETRY supertype, exact concrete-family relationships, NULL/typed-EMPTY distinction, and unchanged Z/M identity.
- Clarifies that header-envelope code/byte order/framing/length and EMPTY compatibility are validated, not numerical agreement of envelope doubles with WKB coordinates; observed/catalog bounds remain an independent comparison. Historical wording is preserved below as history, not a broader current guarantee.
- Records 54 retained red-first regressions (28 failures/26 controls before correction), 399 existing INPN, 327 geometry, 726 combined and 3,721 full-suite passes, four pre-existing full-suite warnings, no skips/xfails, and all required quality gates.
- Binds exact prior-payload equality and unchanged real geometry SHA `997c8c27cbbedb2860778386b3f2eb5afa9f64de6ad07e41fc67b4cec9060ee7`: 15 MULTIPOLYGON/MultiPolygon packages/layers, 11,381 rows, zero prohibited operations, repeated build and independent physical validation.
- Records unchanged public fields, schemas, raw/parser hash definitions, source/catalog/attribute identities and unresolved sig_tadl evidence; no acquisition, environmental semantics, repair, reprojection, or parcel work is introduced.
...
- Status: Complete. Work began after a successful fetch on clean `main` at `9f696c2d687edddff4dc453e195034a7d9b41fac`, equal to `origin/main`. Required native imports, including `pyogrio._io`, succeeded before editing. The locked environment and all four existing INPN source implementations, their four suites, and the source YAML are unchanged.
- One new integration module exposes the frozen `InpnProtectedAreasLayerAlignment` (ten fields), frozen `InpnProtectedAreasEvidenceBundle` (six fields), controlled `InpnProtectedAreasEvidenceError`, `build_inpn_protected_areas_evidence_bundle(extraction, config, catalog, attributes, geometries)`, and `validate_inpn_protected_areas_evidence_bundle(extraction, config, bundle)`. The builder returns the exact bundle; the validator returns `None`. Only these five names are added to the public source exports. No new raw reader, SQL, network path, joined feature table, GeoDataFrame, geometry object, absolute path, bytes, or operational handle is introduced in the bundle.
- Public trust: exact external runtime types and reconstructed strict source configuration precede both existing PUBLIC physical profile validators. Each complete builder invokes `validate_inpn_protected_areas_attribute_profile` once and `validate_inpn_protected_areas_geometry_profile` once, outside layer loops. Those boundaries independently validate extraction/catalog and rebuild their own profiles through the approved readers. Private intrinsic checks establish canonical structure only and are not exported as equivalent trust. The final successful-return postcondition invokes existing PUBLIC `validate_inpn_protected_areas_extraction` after alignment/hash construction; persistent archive/package changes fail before return. Lower source/catalog/attribute/geometry errors are translated with chained causes.
- Alignment retains canonical catalog package/layer order without sorting supplied profiles into compliance. Exact source/archive identity, catalog schema/hash, package/layer inventory, aggregate counts, full `(relative_path, layer_name)` keys, package path/position/size/SHA/driver, and layer identity/position must agree. Catalog feature count, both feature counts and both FID counts are equal; both FID extrema and complete FID-sequence SHA256 are equal. Empty, negative and sparse physical FIDs remain supported; neither contiguity nor `id_mnhn` is substituted. The public validator first checks exact bundle/schema/alignment/hash closure, then physically rebuilds through the public builder and exact-compares the complete result.
- The two existing FID algorithms were inspected directly: the attribute reader accepts Python/NumPy integral FIDs, rejects Boolean/nonintegral/duplicates and converts accepted FIDs to exact built-in integers; the SQLite geometry reader already requires exact built-in integer FIDs and rejects duplicates. Both sort numerically and hash the same JSON integer list, using UTF-8, `sort_keys=True`, `separators=(",", ":")`, `ensure_ascii=False`, `allow_nan=False`. Neither algorithm was copied into the integration module. A permanent structural regression proves `[1, 2, 4]` versus `[1, 3, 4]` fails despite identical count/min/max `(3, 1, 4)` and valid rehashed intrinsic geometry evidence; reversed physical attribute read order still aligns.
- New bundle schema 1 hashes only `evidence_bundle_schema_version`, the catalog/attribute/geometry schema-plus-complete-SHA subobjects, and all ten fields of every ordered alignment record. Large upstream value domains are retained through the original immutable profiles and transitively committed, not redundantly expanded in the bundle payload. Absolute cache roots, cache-hit flags, timestamps and runtime durations are excluded. The bundle inherits the geometry profile's toolchain dependence; no platform/toolchain-independent geometry claim is made. Existing catalog schema 2, attribute schema 1, geometry schema 1, geometry encoding, acquisition/extraction schemas 1 and all prior payload/hash definitions remain unchanged.
- Permanent new inventory: 25 test functions expand to 141 cases. Real temporary ZIP/GPKG positive controls exercise both approved physical validators, determinism, independent validation, same names in distinct packages, multiple layers, empty/negative/sparse FIDs, reversed read order, actual M/ZM ordinates and cache-root/cache-hit portability. Adversarial coverage includes exact public and nested types, same-class forged config reconstruction before physical calls, all identity/inventory/count/range/hash contradictions, coherently rehashed upstream/catalog/alignment/bundle forgeries, post-alignment persistent archive/package changes, lower-error chaining, exact validator ownership/counts, public exports and recursive immutable portable leaves. Impossible profile combinations are explicitly labeled pure structural tests, never mocked physical proof. All ten alignment scalar-type cases specifically require `noncanonical runtime type`, so stale hashes cannot mask missing type checks.
- Authoritative focused runs used `uv run pytest -q` and a fresh SHORT unique `--basetemp` under `%LOCALAPPDATA%\LandScout\pytest-runs` each time. Existing four files (`test_inpn_protected_areas_fr.py`, `test_inpn_protected_areas_catalog_fr.py`, `test_inpn_protected_areas_attributes_fr.py`, `test_inpn_protected_areas_geometry_fr.py`, all under `tests/unit/`) passed 803 in 24.08 seconds before implementation. The finalized new `tests/unit/test_inpn_protected_areas_evidence_fr.py` passed 141 in 19.42 seconds; all five files together passed 944 (`803 + 141`) in 41.61 seconds. Every final focused run completed cleanup with exit code 0, zero unhandled warnings, no skips and no xfails.
- Full repository: `uv run pytest -q --basetemp <fresh short isolated path>` completed cleanup with exit code 0: 3,939 passed (`3,798 + 141`) in 777.44 seconds. No skips or xfails. Exactly four pre-existing UserWarnings remain: missing-CRS warnings in `test_assess_grid_coverage.py::test_invalid_coverage_geometry_is_rejected[geometry0-None-CRS]`, `test_enrich_planning_features.py::test_missing_crs_is_rejected[source]`, and `test_normalize_access_ign.py::test_wrong_or_missing_road_crs_is_rejected[None]`; the GeoSeries.notna EMPTY-behavior warning in `test_enrich_planning_features.py::test_null_or_empty_source_geometry_is_rejected[geometry1]`. No new warning was introduced.
- Real EP verification used existing byte-verified download and extraction cache hits, without source edits or downloads. Archive size remains `99,835,011` bytes; archive SHA remains `73688bc37205a5e7f59e2065a0b81fc8cf2a242bdec5d7d2786f083671c4abe5`. Fresh public builds retain catalog SHA `ba1b9be89d6b951a5c3b5d6b54d1c42f14e0c7bc6669079b1944ff2ffd4c6b34`, attribute SHA `c0bfb73643f2143bd050a7b3f6f59e7ddb52cbcd0efe8612cc45adbc8bc254e8`, and geometry SHA `997c8c27cbbedb2860778386b3f2eb5afa9f64de6ad07e41fc67b4cec9060ee7`. The entire geometry JSON-compatible payload equals the recorded approved payload on the same toolchain. Attribute evidence was physically rebuilt and checked against the approved SHA; no unavailable historical attribute JSON comparison is claimed.
- Observed bundle SHA: `a558236c6a246999fca83ac10c443cd686807675327cdab2da5405143db56016`. Repeated complete construction and independent PUBLIC bundle validation both succeeded and reproduced the same bundle/hash; a separate compact-payload recomputation also matched. Exactly 15 packages, 15 aligned layer pairs and 11,381 matched FIDs; computed missing-layer, extra-layer and FID-mismatch totals are all zero. Every per-layer attribute/geometry hash pair below was observed equal; no expected new hash or per-phase read count was imposed in advance.
- Actual native toolchain matched the recorded geometry evidence before comparison: Python `3.12.13` (MSC v.1944 64-bit AMD64), SQLite `3.53.1`, Pyogrio `0.13.0`, GDAL `3.12.4`, Shapely `2.1.2`, GEOS `3.13.1`, PyProj `3.7.2`. Real audit total runtime was 190.086 seconds, exit code 0, zero unhandled warnings. The unchanged geometry payload still records 11,381 valid non-empty XY MultiPolygon rows and 10,281,493 coordinates, with NULL/EMPTY/invalid/Z/M all zero and all 15 observed bounds matching catalog bounds. These matches do not resolve the existing `sig_tadl` CRS/coordinate observation.

Observed canonical layer alignment (package positions 0..14, all layer positions 0; each row count equals both FID counts; all observed FID minima are 1 and maxima equal row count):

| Package / layer key | Rows | Attribute FID SHA256 | Geometry FID SHA256 | Match |
|---|---:|---|---|---|
| `EP/sig_blm.gpkg:sig_blm` | 4 | `f6bd10506e9a4daed7c03eda2f2fde54be3bd58eee49dab471c18a888ffbdb6f` | `f6bd10506e9a4daed7c03eda2f2fde54be3bd58eee49dab471c18a888ffbdb6f` | exact |
| `EP/sig_cli.gpkg:sig_cli` | 1 | `080a9ed428559ef602668b4c00f114f1a11c3f6b02a435f0bdc154578e4d7f22` | `080a9ed428559ef602668b4c00f114f1a11c3f6b02a435f0bdc154578e4d7f22` | exact |
| `EP/sig_epa.gpkg:sig_epa` | 5 | `f5baf0e4336fd53b4c82b453ece859868475160d36f22e9551a0e9b10ac9cc00` | `f5baf0e4336fd53b4c82b453ece859868475160d36f22e9551a0e9b10ac9cc00` | exact |
| `EP/sig_glp.gpkg:sig_glp` | 94 | `0e23b6e4dfbf50733e1b02554bf2e094265eaa4482c886717fbfb47cd042c0e2` | `0e23b6e4dfbf50733e1b02554bf2e094265eaa4482c886717fbfb47cd042c0e2` | exact |
| `EP/sig_guf.gpkg:sig_guf` | 42 | `9b669fa350a15f13d4ab456869612801860848d589df68cb61930c16e5ef4a98` | `9b669fa350a15f13d4ab456869612801860848d589df68cb61930c16e5ef4a98` | exact |
| `EP/sig_maf.gpkg:sig_maf` | 21 | `d2592aec818c569765bfa500e771578e176f38be15ddf0a61a24fdab79b09876` | `d2592aec818c569765bfa500e771578e176f38be15ddf0a61a24fdab79b09876` | exact |
| `EP/sig_metrop.gpkg:sig_metrop` | 10872 | `74fbd5bd2940405f95e1adc81075f31edb5a55856a0c193ddc6ec01c5e4cc303` | `74fbd5bd2940405f95e1adc81075f31edb5a55856a0c193ddc6ec01c5e4cc303` | exact |
| `EP/sig_mtq.gpkg:sig_mtq` | 79 | `4e5d751543e602052a0eea21a6538e84127b2ccb12f680679cb5dac5ffef2bed` | `4e5d751543e602052a0eea21a6538e84127b2ccb12f680679cb5dac5ffef2bed` | exact |
| `EP/sig_myt.gpkg:sig_myt` | 30 | `ebda43c0ca7bab4c6b235bede1b8d289a4181c5bbe556cf1af17979956289a7b` | `ebda43c0ca7bab4c6b235bede1b8d289a4181c5bbe556cf1af17979956289a7b` | exact |
| `EP/sig_ncl.gpkg:sig_ncl` | 88 | `850993d9f399eb243af2ee50618b6f037d3735bffd26d0c62887e83ff79f6e7e` | `850993d9f399eb243af2ee50618b6f037d3735bffd26d0c62887e83ff79f6e7e` | exact |
| `EP/sig_pyf.gpkg:sig_pyf` | 87 | `44518a20100d2f73652ce108d7db6acbbec00b30bffc4768e56f59e13f451146` | `44518a20100d2f73652ce108d7db6acbbec00b30bffc4768e56f59e13f451146` | exact |
| `EP/sig_reu.gpkg:sig_reu` | 44 | `a5b6e06588c6904f0b803dbafe19b86b09846118261ed22e81d06c4db1217d60` | `a5b6e06588c6904f0b803dbafe19b86b09846118261ed22e81d06c4db1217d60` | exact |
| `EP/sig_spm.gpkg:sig_spm` | 2 | `49a64717d5d4cb19952e6eac2946415cf6879adacf9908e7d872332d32c6e684` | `49a64717d5d4cb19952e6eac2946415cf6879adacf9908e7d872332d32c6e684` | exact |
| `EP/sig_subant.gpkg:sig_subant` | 11 | `e1330a9fe8a1883cb0bdc41e4fb5d25cf1c4fcd4c48b2c360d6da717513e1e4e` | `e1330a9fe8a1883cb0bdc41e4fb5d25cf1c4fcd4c48b2c360d6da717513e1e4e` | exact |
| `EP/sig_tadl.gpkg:sig_tadl` | 1 | `080a9ed428559ef602668b4c00f114f1a11c3f6b02a435f0bdc154578e4d7f22` | `080a9ed428559ef602668b4c00f114f1a11c3f6b02a435f0bdc154578e4d7f22` | exact |

Observed read calls by phase (metadata columns count calls, not feature rows; SQLite snapshots, exact FID/BLOB SELECTs and geometry-layer reader calls were equal within each phase):

| Phase | Seconds | list_layers | read_info | Attribute reads / rows | SQLite snapshots / FID+BLOB SELECTs / rows |
|---|---:|---:|---:|---:|---:|
| source_cache | 1.982 | 0 | 0 | 0 / 0 | 0 / 0 / 0 |
| catalog_build | 3.038 | 15 | 15 | 0 / 0 | 0 / 0 / 0 |
| attribute_profile_build | 15.684 | 45 | 45 | 15 / 11381 | 0 / 0 / 0 |
| geometry_profile_build | 29.951 | 45 | 45 | 0 / 0 | 15 / 15 / 11381 |
| evidence_bundle_build | 46.682 | 90 | 90 | 15 / 11381 | 15 / 15 / 11381 |
| evidence_bundle_repeat | 46.362 | 90 | 90 | 15 / 11381 | 15 / 15 / 11381 |
| evidence_bundle_independent_validation | 46.386 | 90 | 90 | 15 / 11381 | 15 / 15 / 11381 |
| Total | 190.086 | 375 | 375 | 60 / 45,524 | 60 / 60 / 45,524 |

- Integrated reader audit allowed only exact approved attribute-only Pyogrio calls: immutable verified package bytes, catalog layer/ordered columns, `read_geometry=False`, `fid_as_index=True`, `use_arrow=False`, `datetime_as_string=True`. Geometry remained the existing verified immutable SQLite snapshot with query-only/trusted-schema settings and exact FID-plus-BLOB projection. Build, repeat and independent validation have separate observed counters above. Fatal guards report DNS 0, HTTP 0, downloads 0, alternate feature readers 0, Pyogrio geometry materialization 0, repairs/normalization/geometry transformations 0, reprojection 0 and parcel/spatial analysis 0. Audit scripts and the JSON report are outside the repository and excluded from the commit.
- Final quality commands all succeeded: `uv run ruff check .`; `uv run ruff format --check .` (109 files); `uv run mypy src` (52 source files); `uv lock --check` (48 packages); `uv pip check` (45 compatible packages); `uv run python -m compileall -q src tests`; `git diff --check`. Both required literal conflict-marker `git grep -n` searches returned exit code 1 with no matches. Documentation review and raw-SHA audits passed for 114 companions; new source/test and export companions have exact complete source snapshots, qualified signature/field/assertion/parameter inventories and valid links. New source has eight functions, three classes and sixteen fields; test inventory has 47 qualified functions, three classes and six fields. Full diff review found only the intended fourteen source/test/export/documentation paths. This log is finalized last, then its SHA-bound complete-snapshot companion is synchronized; prior historical entries are preserved unchanged.
- Scope of proof: source/profile validation and physical FID alignment only. Matching FID commitments are cryptographic evidence, not a raw per-feature attribute/geometry join, correct georeferencing, protected-area category meaning, legal regime or parcel policy. No collection-assignability/reader correction, source identity/config/schema migration, geometry repair/normalization/reprojection, Natura 2000, ZNIEFF, parcel loading/intersection, exclusion, score, ranking, CI/PR workflow or further functional step is added. Independent review comes next.

## STEP 7F.1B.3.2 — Correct collection subtype assignability

- Status: Complete. Work began after a successful fetch on clean `main` at `a2ebe5266a37cb8de6131b52631111e47e0235f3`, equal to `origin/main`.
- The preceding STEP 7F.1B.3.1 implementation correctly followed its ticket. Independent review identified an error in that review-authored ticket: equality with every non-GEOMETRY declaration was too restrictive for GEOMETRYCOLLECTION. This correction restores the supported core subtype hierarchy in [GeoPackage 1.4 Requirement 32 and normative Annex G](https://www.geopackage.org/spec140/), not a new geometry reader or environmental feature.
- One private explicit `MappingProxyType[str, frozenset[str]]`, backed by its own unaliased inline dictionary, is the shared assignability relation for raw-WKB parsing and intrinsic profile validation. GEOMETRY accepts all seven supported roots; GEOMETRYCOLLECTION accepts GeometryCollection, MultiPoint, MultiLineString and MultiPolygon; the other six declarations accept only their matching family. Bare Point/LineString/Polygon remain forbidden under GEOMETRYCOLLECTION, and Polygon/MultiPolygon are never interchangeable. The independent 8-by-7 matrix contains 17 accepted and 39 rejected pairs.
- Exact built-in uppercase declaration vocabulary and exact geometry-column SQL/declaration equality remain separate mandatory gates. A GEOMETRYCOLLECTION metadata declaration still requires SQL GEOMETRYCOLLECTION even when rows contain MultiPolygon. No geometry is wrapped, converted or renamed. WKB root ID 0 and non-linear/extension roots remain forbidden. NULL has no root but still requires a valid declaration; EMPTY retains its root; Z/M does not change assignability.
- Red-first execution against untouched starting production: exactly 3 failures, 53 passes and 327 deselections in 0.95 seconds. GEOMETRYCOLLECTION/MultiPoint, GEOMETRYCOLLECTION/MultiLineString and GEOMETRYCOLLECTION/MultiPolygon each raised the controlled “not assignable” error despite the independent accepted expectation. The other 53 decisions remained correct. The corrected raw-parser matrix passed all 56 in 0.58 seconds.
- Permanent new cases total 77: real ISO-WKB matrix 56; individual intrinsic rehashed domains 6; mixed intrinsic rehashed domains 2; physical public Multi* build plus independent validation 1; physical bare-Polygon rejection 1; exact SQL mismatch rejection 1; EMPTY XY Multi* 3; genuine non-empty M/ZM Multi* 6; forbidden root ID 0 control 1. All 327 existing cases remain unchanged. M/ZM tests assert every actual ordinate, dimension, root and ISO-WKB roundtrip; existing measured POINT/POINT EMPTY and dimension-loss regressions remain. No measured-empty multi-collection redesign is introduced.
- The first complete 77-case development run reached 71 passes and six fixture-setup failures caused by Windows path length under an overly long isolated basetemp prefix, before catalog/geometry execution. Short unique prefixes under the same required isolation root resolved that harness issue without production changes. The finalized new-regression run passed 77 with 327 deselected in 1.56 seconds, zero unhandled warnings, no skips and no xfails.
- Focused commands, each with a fresh unique `--basetemp` below `%LOCALAPPDATA%\LandScout\pytest-runs`: `uv run pytest -q tests/unit/test_inpn_protected_areas_geometry_fr.py -k collection_subtype --basetemp <unique path>` passed 77; `uv run pytest -q tests/unit/test_inpn_protected_areas_fr.py tests/unit/test_inpn_protected_areas_catalog_fr.py tests/unit/test_inpn_protected_areas_attributes_fr.py --basetemp <unique path>` passed the existing 399 in 10.69 seconds; the complete geometry-file command passed 404 (`327 + 77`) in 13.08 seconds; the four-file combined INPN command passed 803 (`726 + 77 = 399 + 404`) in 22.62 seconds. All authoritative focused runs completed cleanup and returned exit code 0 with zero unhandled warnings.
- Full repository command: `uv run pytest -q --basetemp <new unique isolated path>` completed cleanup and returned exit code 0: 3,798 passed (`3,721 + 77`) in 752.29 seconds, no skips or xfails. Exactly four pre-existing UserWarnings remain: one missing-CRS warning each in `test_assess_grid_coverage.py::test_invalid_coverage_geometry_is_rejected[geometry0-None-CRS]`, `test_enrich_planning_features.py::test_missing_crs_is_rejected[source]`, and `test_normalize_access_ign.py::test_wrong_or_missing_road_crs_is_rejected[None]`; one GeoSeries.notna EMPTY-behavior warning in `test_enrich_planning_features.py::test_null_or_empty_source_geometry_is_rejected[geometry1]`.
- Warning contracts: the existing all-NULL mandatory Z/M metadata fixture still explicitly captures eight expected Pyogrio RuntimeWarnings. The new SQL/declaration mismatch negative explicitly captures six GDAL RuntimeWarnings: two during catalog construction and four during build preflight, before the precise SQL equality error. They are asserted individually by category/message and exact group counts, not globally suppressed; none escapes focused warning summaries.
- Independent AST review proved the only changed existing production nodes are `_require_gpkg_geometry_type` and `_require_geometry_assignable`, with only the private immutable relation added. All existing test definitions/constants are unchanged. Public dataclass fields, profile payload, canonical JSON, raw-BLOB/parser-WKB/complete-profile hash functions, FIDs, CRS/SRS/bounds, SQLite snapshot/query/WKB/dimensional readers, and source-complete revalidation/postconditions are unchanged. Geometry-profile schema 1, geometry-encoding schema 1 and `SHAPELY_EXTENDED_WKB_LITTLE_ENDIAN_SOURCE_DIMENSION_NO_SRID_V1` remain unchanged; catalog schema 2, attribute schema 1 and acquisition/extraction schemas 1 are unchanged.
- Before real-source comparison, the native toolchain was checked against the recorded payload and matched: Python 3.12.13 (MSC v.1944 64-bit AMD64), SQLite 3.53.1, Shapely 2.1.2, GEOS 3.13.1, Pyogrio 0.13.0, GDAL 3.12.4 and PyProj 3.7.2. No baseline was regenerated or silently migrated.
- Controlled offline real EP verification used byte-verified download/extraction cache hits only. The archive remains 99,835,011 bytes with SHA `73688bc37205a5e7f59e2065a0b81fc8cf2a242bdec5d7d2786f083671c4abe5`; fresh schema-2 catalog SHA remains `ba1b9be89d6b951a5c3b5d6b54d1c42f14e0c7bc6669079b1944ff2ffd4c6b34`. All 15 packages/layers still declare MULTIPOLYGON and report actual MultiPolygon: 11,381 valid non-empty XY rows, 10,281,493 coordinates, NULL/EMPTY/invalid/Z/M all zero, and all 15 observed bounds exactly matching catalog bounds. The prior unresolved `sig_tadl` CRS/bounds observation remains unchanged.
- Complete real JSON-compatible profile payload equality, not merely hash equality, passed against the approved payload. Geometry SHA is unchanged on the same toolchain: `997c8c27cbbedb2860778386b3f2eb5afa9f64de6ad07e41fc67b4cec9060ee7`. Repeat build equality and independent public physical validation passed. The controlled sequence used 45 query-only snapshots, 45 exact FID-plus-BLOB SELECTs and 34,143 rows across three passes. First build: 28.957 seconds; complete offline setup/build/repeat/validation: 91.199 seconds; exit code 0.
- Fatal sentinels plus exact query/cache-hit assertions prove DNS 0, HTTP 0, downloads 0, attribute-column reads 0, Pyogrio/GeoPandas geometry-row reads 0, repairs/normalization/spatial operations 0 and reprojection 0. The approved attribute schema-1 SHA remains `c0bfb73643f2143bd050a7b3f6f59e7ddb52cbcd0efe8612cc45adbc8bc254e8`; attribute source/config/schema/payload are untouched and attributes were deliberately not reread. Audit scripts/reports and caches remain outside the commit.
- Quality gates completed successfully: `uv run ruff check .`; `uv run ruff format --check .` (107 files); `uv run mypy src` (51 source files); `uv lock --check` (48 packages); `uv pip check` (45 compatible packages); `uv run python -m compileall -q src tests`; `git diff --check`; unresolved conflict-marker searches. The two geometry companions and necessary current environment/trust/testing sections were synchronized; raw source SHA, complete snapshots and symbol/assertion inventories were verified. This log is finalized last, its companion is then synchronized, and historical entries are retained unchanged.
- Envelope boundary remains factual: code/byte order/framing/length are validated; numerical agreement between header-envelope doubles and WKB coordinates is not newly implemented. No acquisition, extraction, catalog/attribute code, config, dependency, public export, source identity, payload/hash contract, geometry algorithm or other LandScout feature changes. No category/legal semantics, Natura 2000, ZNIEFF, parcels/intersections, exclusion, scoring, ranking, CI or pipeline changes.

## STEP 7F.1B.3.1 — Close GeoPackage geometry-type contracts

- Status: Complete. Work began on clean `main` at `57edf93611d028092450a58de1b6df73bc6a1ee2`, equal to `origin/main`; native SQLite deserialization/query-only/trusted-schema and Shapely M/ZM/EMPTY preflight passed.
- Independent review found three unclosed physical type relationships: canonical GeoPackage declaration, matching SQL geometry-column declaration, and assignable actual WKB root. [GeoPackage 1.4 requirements 25, 31 and 32](https://www.geopackage.org/spec140/) were inspected before correction.
- One private immutable `MappingProxyType`, backed by its own inline dictionary and immutable tuple leaves, now defines the exact core vocabulary: `GEOMETRY`, `POINT`, `LINESTRING`, `POLYGON`, `MULTIPOINT`, `MULTILINESTRING`, `MULTIPOLYGON`, `GEOMETRYCOLLECTION`. Concrete declarations map to their exact Shapely names. Declaration-only ID 0 does not broaden the unchanged raw ISO WKB parser's accepted IDs 1..7.
- Metadata now rejects non-built-in strings, empty/edge-whitespace/lowercase/unknown/non-linear declarations, and binds the exact physical geometry-column SQL declared type from `PRAGMA table_info` to `gpkg_geometry_columns.geometry_type_name`. No casefolding, coercion, normalization or SQL-type guessing is permitted.
- After complete root WKB framing and before Shapely conversion, `GEOMETRY` permits any supported core root; each specific declaration permits only its matching family. NULL has no root to compare; EMPTY retains its type; XY/XYZ/XYM/XYZM does not change assignability. Intrinsic profile validation uses the same relation and validates declarations even for NULL-only layers, rejecting contradictory declaration/type domains despite a recalculated complete-profile hash.
- Envelope wording is explicitly narrowed: GeoPackageBinary envelope code, byte order, framing, length and EMPTY/code compatibility are checked; numerical header-envelope doubles are not verified against WKB coordinates. Observed bounds are independently calculated from parsed coordinates and compared with physical catalog bounds. This clarifies the earlier broad historical “envelopes” wording without changing that historical record or adding numerical-envelope behavior.
- Schemas/payloads/hash contracts are unchanged: geometry-profile schema 1, geometry-encoding schema 1, encoding `SHAPELY_EXTENDED_WKB_LITTLE_ENDIAN_SOURCE_DIMENSION_NO_SRID_V1`, catalog schema 2, attribute schema 1, and acquisition/extraction schema 1. Public dataclass fields, FID representation, canonical JSON, raw BLOB stream, parser-derived WKB stream, complete profile hashing, SQLite byte reader, and source-complete revalidation/postconditions are unchanged.
- Red-first evidence against untouched starting production: the 54 added cases produced 28 failures, 26 passes and 273 deselections in 2.12 seconds (24 fixture warnings before fixture-only metadata alignment). All 273 existing cases remain. Five PK-error fixtures now declare conforming `geom GEOMETRY` so the new SQL-type check cannot shadow their original PK regressions; new POINT fixtures explicitly declare XY metadata.
- Permanent new case inventory (54): invalid physical declarations 6; SQL binding 4 physical mismatches (including lowercase) plus 3 exact-runtime-type cases; specific WKB root mismatches 8 (including EMPTY); matching specific roots 7; `GEOMETRY` supertype controls 7; public POINT NULL/EMPTY controls 2; dimensional POINT controls 8; intrinsic invalid NULL-only declarations 6; intrinsically rejected rehashed declaration/domain contradictions 3.
- Controlled offline real EP verification: byte-verified download and extraction cache hits; unchanged 99,835,011-byte archive SHA `73688bc37205a5e7f59e2065a0b81fc8cf2a242bdec5d7d2786f083671c4abe5`. All 15 packages/layers pass exact `MULTIPOLYGON` metadata/SQL binding and have only observed `MultiPolygon`; 11,381 non-empty valid XY rows and 10,281,493 coordinates remain unchanged, with zero NULL/EMPTY/invalid/Z/M and 15 exact catalog/observed-bounds matches. The prior unresolved `sig_tadl` CRS/bounds observation remains untouched.
- Exact fresh catalog SHA remains `ba1b9be89d6b951a5c3b5d6b54d1c42f14e0c7bc6669079b1944ff2ffd4c6b34`. The approved attribute SHA remains `c0bfb73643f2143bd050a7b3f6f59e7ddb52cbcd0efe8612cc45adbc8bc254e8`; its source/config/schema/payload are untouched, and attributes were deliberately not reread in this zero-attribute verification.
- Geometry SHA remains `997c8c27cbbedb2860778386b3f2eb5afa9f64de6ad07e41fc67b4cec9060ee7`. The complete JSON-compatible profile payload exactly equals the prior approved real payload, not just its hash. Repeated build equality and independent public physical validation passed: 45 query-only SQLite snapshots, 45 exact FID-plus-geometry SELECTs, 34,143 geometry rows across three passes. First build 28.664 seconds; total offline setup/build/repeat/validation 91.489 seconds.
- Fatal sentinels and exact SELECT/cache-hit assertions proved DNS 0, HTTP 0, downloads 0, Pyogrio/GeoPandas feature reads 0, attribute columns 0, repairs/normalization/spatial operations 0 and reprojection 0. Audit scripts and generated reports remain outside the repository and are not committed.
- Focused validation: existing INPN suites 399 passed in 9.77 seconds; geometry suite 327 passed in 12.70 seconds; combined INPN suites 726 passed in 23.21 seconds. No focused warnings, skips or xfails. Each Pytest run used a unique `--basetemp` under `%LOCALAPPDATA%\LandScout\pytest-runs`.
- Full repository validation: `uv run pytest -q --basetemp <unique isolated path>` passed exactly 3,721 cases (`3,667 + 54`) in 738.17 seconds, with four pre-existing UserWarnings: missing-CRS fixtures in `test_assess_grid_coverage`, `test_enrich_planning_features`, and `test_normalize_access_ign`, plus the `GeoSeries.notna` EMPTY warning in `test_enrich_planning_features`. No skips or xfails. The existing INPN, geometry, combined INPN and full suites all exited 0.
- Quality/documentation: repository-wide Ruff check passed; Ruff format check passed (107 files); mypy passed (51 source files); uv lock check passed (48 packages); uv pip check passed (45 compatible packages); compileall, git diff check and both conflict-marker searches passed. Geometry source/test companions, environment/trust/testing cross-documents, this entry and its companion were synchronized; exact source-SHA/snapshot and heading/function inventory audits passed.
- Boundary: no category/legal semantics, Natura 2000, ZNIEFF, geometry repair/normalization/reprojection, parcel loading/intersection, exclusion, score or ranking. This is a geometry-type conformance correction only; no acquisition/cache/extraction, source identity, attribute contract, geometry/CRS/bounds algorithm, or other LandScout domain changed.
...
- Controlled real-source verification: a read-only run reused the approved cache with DNS, safe HTTPS, and every geometry-capable reader replaced by immediate failures. Download and extraction were cache hits; DNS, HTTP, downloads, geometry-reader calls, and GeoDataFrames were all zero. The complete builder plus independent public validator made 30 expected attribute reads across two 15-layer passes. The result remains exactly 15 packages, 15 layers, 195 fields, and 11,381 rows. Catalog schema/hash remain `2` / `ba1b9be89d6b951a5c3b5d6b54d1c42f14e0c7bc6669079b1944ff2ffd4c6b34`; attribute schema/hash remain `1` / `c0bfb73643f2143bd050a7b3f6f59e7ddb52cbcd0efe8612cc45adbc8bc254e8`.
- Tests: existing INPN source/catalog suites passed 258 cases in 4.20 seconds; the final attribute suite passed 115 cases in 5.75 seconds; all three INPN files passed 373 cases in 9.42 seconds, with no focused warnings. The complete repository passed exactly 3,368 cases (`3,327 + 41`) with four pre-existing warnings in 723.31 seconds. Every Pytest invocation used a unique `--basetemp` below `%LOCALAPPDATA%\LandScout\pytest-runs`.
- Quality and documentation: `uv run ruff check .` passed; `uv run ruff format --check .` reported 105 formatted files; `uv run mypy src` reported no issues in 50 source files; `uv lock --check` resolved 48 packages; `uv pip check` verified 45 compatible packages; `uv run python -m compileall -q src tests`, `git diff --check`, and both conflict-marker searches passed. The source/test companions contain exact current snapshots and bind SHA256 `289323cbeea74ee9166449448306f4792e335f42c710805783f1b9d539dd4094` and `e8118609038f799be63abec16f8b38f2a0386eecdd9c1534497e4bf39d124fef`; cross-cutting hash/intrinsic claims were corrected without rewriting historical STEP 7F.1B.2 evidence.
- Boundary: EP source semantics, archive/source identity, cache/extraction schemas and layout, catalog schema/hash, attribute schema/hash, raw value domains, Pyogrio reader options, and physical metadata are unchanged. No category or legal-regime semantics, Natura 2000 or ZNIEFF mapping, EP geometry load, parcel relation, environmental exclusion, score, or ranking was added.
