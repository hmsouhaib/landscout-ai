...
- Geometry metadata accepts only exact built-in uppercase core declarations: `GEOMETRY`, `POINT`, `LINESTRING`, `POLYGON`, `MULTIPOINT`, `MULTILINESTRING`, `MULTIPOLYGON`, and `GEOMETRYCOLLECTION`. The physical geometry-column SQL declared type must exactly equal `gpkg_geometry_columns.geometry_type_name`, including when a collection-declared column stores Multi* rows. Separately, the raw ISO WKB root follows one immutable assignability relation: any supported core root for `GEOMETRY`; `GeometryCollection`, `MultiPoint`, `MultiLineString`, or `MultiPolygon` for the second supported supertype `GEOMETRYCOLLECTION`; only the matching Shapely family for each other declaration. Bare `Point`, `LineString`, and `Polygon` are not assignable to `GEOMETRYCOLLECTION`, and `Polygon`/`MultiPolygon` remain distinct. There is no casefold equivalence, coercion, geometry wrapping/renaming, extended-type support, or dimension-based type substitution. NULL has no root, while EMPTY retains its family. Intrinsic validation repeats the same declaration/observed-domain relationship, rejecting contradictions even after a coherent complete-profile rehash.
- Geometry BLOBs are exact Standard GeoPackageBinary evidence, not naked WKB. Header validation checks magic/version/flags, envelope code/byte order/framing/length, SRS, and empty-state evidence; only embedded WKB reaches Shapely. Numerical header-envelope values are not compared against WKB coordinates. Parsed XY/XYZ/XYM/XYZM state must respect source Z/M declarations and retain every present finite ordinate. Observed bounds are independently calculated from parsed coordinates and compared with physical catalog bounds. NULL/EMPTY/non-empty and valid/invalid topology remain factual domains with complete reasons and exact observed/catalog bound relations; no repair, coordinate transformation, or semantic category selection occurs.
...
- The evidence bundle retains the exact immutable catalog, attribute profile, and geometry profile only after both existing public profile validators physically reconstruct their evidence. Private structural alignment checks are not an exported alternative to this source-complete boundary. The independent bundle validator checks exact nested types/hash closure, repeats the two physical validators, rebuilds the expected bundle, and exact-compares it. Lower-layer controlled failures are chained into `InpnProtectedAreasEvidenceError`; final extraction revalidation rejects persistent source mutation after profile validation.
- One alignment record is required for every catalog layer in unchanged package/layer order under the full `(relative_path, layer_name)` identity. Source/archive/catalog lineage and repeated package path/position/size/SHA/driver must agree, as must layer position, all feature/FID counts, FID extrema, and the complete FID-sequence SHA. Both profilers hash the same sorted physical integer-FID JSON sequence; equal counts/extrema alone cannot hide `[1, 2, 4]` versus `[1, 3, 4]`. Sparse, negative, and empty domains remain governed by the existing intrinsic FID rules, not a new contiguity or business-identifier rule.
- Bundle schema 1 hashes a compact canonical JSON payload of its schema, each upstream schema and complete hash, and the complete ordered alignment records. Upstream domains are retained in their original immutable models, not redundantly serialized into the bundle hash. Existing schemas/payloads/hashes are unchanged; geometry toolchain dependence is transitively retained. A coordinated upstream/alignment/bundle rehash proves only internal consistency until public physical validation succeeds. Portable relative paths remain evidence, while new bundle/alignment records retain no absolute filesystem paths or operational handles.
- The current chain ends at factual EP attribute/geometry profiles and their source-bound physical FID alignment. It does not join raw attribute cells to geometries or prove category/CRS correctness. EP is distinct from the separate Natura 2000 and ZNIEFF reference archives. It does not interpret categories or legal regimes, normalize environmental geometry, or perform parcel analysis.
