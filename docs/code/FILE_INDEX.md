...
| `src/landscout/common/immutable_mapping.py` | Provides a deeply immutable mapping value for frozen decision-input configuration and policy models. | [files/src/landscout/common/immutable_mapping.py.md](files/src/landscout/common/immutable_mapping.py.md) |
...
| Repository file | Actual role | Companion |
|---|---|---|
| `src/landscout/sources/__init__.py` | Re-exports approved source-bound adapter APIs, including the Cadastre parcel source boundary, without presenting raw-path helpers as equivalent trust roots. | [files/src/landscout/sources/__init__.py.md](files/src/landscout/sources/__init__.py.md) |
...
| `tests/unit/test_deep_immutability.py` | Proves that every loaded trust-bearing configuration/policy family has no reachable mutable collection, rejects all relevant in-place mutation operations, isolates caller aliases, and preserves canonical hashes. | [files/tests/unit/test_deep_immutability.py.md](files/tests/unit/test_deep_immutability.py.md) |
