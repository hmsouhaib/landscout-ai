...
The geometry suite contains 404 collected cases (327 retained + 77 collection-subtype regressions); the combined INPN suites contain 803 (399 existing + 404 geometry). Completed execution results are recorded in `docs/DEV_LOG.md`. One all-NULL mandatory Z/M metadata fixture explicitly expects eight Pyogrio measured-type metadata warnings. A separate collection-declaration/SQL-type mismatch negative explicitly expects two catalog and four build RuntimeWarnings before the exact SQL binding fails. These warnings are asserted, not hidden; real geometry rows still bypass Pyogrio entirely.
...
On this Windows environment, every Pytest invocation supplies a fresh unique `--basetemp` under `%LOCALAPPDATA%\LandScout\pytest-runs`; the broken default `pytest-current` path is not reused. STEP 7F.1B.3.2 runs the new collection-subtype regressions first, then the 399-case existing INPN baseline, the 404-case geometry suite, combined INPN suites, and the complete repository (starting baseline 3,721 plus 77 subtype cases).
