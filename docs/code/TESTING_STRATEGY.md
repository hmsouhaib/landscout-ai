...
The geometry suite contains 327 collected cases (273 retained + 54 core type-contract regressions); the combined INPN suites contain 726 (399 existing + 327 geometry). The focused geometry and combined runs report zero unhandled warnings. One all-NULL mandatory Z/M metadata fixture explicitly expects eight Pyogrio measured-type metadata warnings; real geometry rows still bypass Pyogrio entirely.
...
On this Windows environment, every Pytest invocation supplies a fresh unique `--basetemp` under `%LOCALAPPDATA%\LandScout\pytest-runs`; the broken default `pytest-current` path is not reused. STEP 7F.1B.3.1 runs the 399-case existing INPN baseline first, then the 327-case geometry suite, combined INPN suites, and the complete repository (starting baseline 3,667 plus 54 type-contract cases).
