...
__all__ = ["resolve_planning_feature_codes"]

PROFILE_SCHEMA_VERSION = 1
RESULT_HASH_SCHEMA_VERSION = 1
STANDARD_MODEL = "CNIG PLU v2017"
OFFICIAL_GPU_HOST = "www.geoportail-urbanisme.gouv.fr"
...
def _official_url(value: object, label: str) -> str:
    ...
    if (
        parsed.scheme != "https"
        or parsed.hostname != OFFICIAL_GPU_HOST
        or parsed.username is not None
        or parsed.password is not None
        or parsed.fragment
    ):
        raise ValueError(f"{label} must use HTTPS on the official GPU host")
    return text
...
class OfficialSourceUrls(_StrictModel):
    ...
    def _validate_urls(self) -> OfficialSourceUrls:
        _official_url(self.prescription, "prescription source URL")
        _official_url(self.information, "information source URL")
        return self
...
class CnigFeatureCodeRecord(_StrictModel):
    ...
    def _validate_record(self) -> CnigFeatureCodeRecord:
        ...
        _exact_string(self.official_label, "official label")
        _optional_exact_string(self.legal_reference, "legal reference")
        _optional_exact_string(
            self.regulation_or_annex_reference,
            "regulation or annex reference",
        )
        _official_url(self.official_source_url, "record official source URL")
        return self
...
class CnigFeatureCodeProfile(_StrictModel):
    ...
    def _validate_profile(self) -> CnigFeatureCodeProfile:
        ...
        for record in self.records:
            expected_url = (
                self.official_sources.prescription
                if record.feature_family == "PRESCRIPTION"
                else self.official_sources.information
            )
            if record.official_source_url != expected_url:
                raise ValueError("record source URL differs from its official family table")
...
def load_cnig_feature_code_profile(path: str | Path) -> CnigFeatureCodeProfile:
    ...
    try:
        ...
        if not isinstance(payload, Mapping):
            raise PlanningFeatureCodeError("CNIG feature-code profile must be a mapping")
        return CnigFeatureCodeProfile.model_validate(payload)
    except PlanningFeatureCodeError:
        raise
    except Exception as error:
        raise PlanningFeatureCodeError("CNIG feature-code profile is invalid") from error
...
def _resolved_profile(
    ...
    return (
        profile
        if isinstance(profile, CnigFeatureCodeProfile)
        else load_cnig_feature_code_profile(profile)
    )
...
def _planning_standard(document: GpuPlanningDocument) -> str:
    if not isinstance(document, GpuPlanningDocument):
        raise PlanningFeatureCodeError("planning_document must be a GpuPlanningDocument")
    ...
    if len(distinct) != 1:
        raise PlanningFeatureCodeError("Planning document standard lineage is ambiguous")
...
def _validate_feature_catalog(
    ...
    for identifier in identifiers.tolist():
        _strict_string(identifier, f"{label} planning feature ID")
    ...
    families = set(frame["feature_family"].tolist())
    if not families.issubset({"PRESCRIPTION", "INFORMATION"}):
        raise PlanningFeatureCodeError(f"{label} feature family is invalid")
...
def _coded_relations(
    ...
    if not isinstance(relations, pd.DataFrame) or isinstance(relations, gpd.GeoDataFrame):
        raise PlanningFeatureCodeError("Planning-feature relations must be a DataFrame")
    missing = {"parcel_id", "planning_feature_id", *_RELATION_MATCH_COLUMNS}.difference(
        relations.columns
    )
    if missing:
        raise PlanningFeatureCodeError(f"Planning-feature relations lack: {sorted(missing)}")
    ...
    for row in relations.to_dict("records"):
        ...
        if catalog is None or meaning is None:
            raise PlanningFeatureCodeError("Relation references an unknown feature catalog ID")
...
def _canonical_value(value: object) -> object:
    ...
    if isinstance(value, Real):
        ...
        if not math.isfinite(number):
            raise PlanningFeatureCodeError("Integrity payload contains non-finite numeric data")
        return number
...
def _frame_sha256(
    ...
    return _canonical_json_sha256(
        {"domain": domain, **_component_metadata(result), "frame": _frame_payload(frame)}
    )
...
def _result_with_hashes(result: PlanningFeatureCodeResult) -> PlanningFeatureCodeResult:
    ...
    return replace(component, complete_result_content_sha256=_complete_sha256(component))
