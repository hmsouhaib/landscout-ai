...
`load_cadastre_parcels(download)` accepts the exact result object rather than a caller path. `_validate_download` binds its canonical commune to the exact URL produced by `build_cadastre_parcelles_url`, requires the exact official filename/path, regular non-linked gzip, strict positive size, canonical lowercase SHA256, UTC timestamp, and current physical size/SHA. The loader requires a nonempty dataset, not nonempty geometry on every row: null, empty, and invalid polygonal geometries remain representable. It rejects unsupported non-null geometry families and checks `has_z` before returning `CadastreParcelSource(download, parcels)`. Its before/after path checks detect ordinary file changes but are not an immutable byte snapshot passed to the parser.
...
`filter_parcels_by_area` reconstructs and revalidates an exact immutable `ParcelConfig` before masks, rejects a pre-existing `rejection_reason`, and invokes the shared canonical parcel validator. That validator requires the exact 12-column prefix, IDs, WGS84 geometry/status facts, a finite positive EPSG:2154-recomputed area for every `VALID` row, and null area for every `INVALID` row. Candidates preserve the input schema. Rejected rows append `rejection_reason`: `INVALID_GEOMETRY`, `AREA_BELOW_MIN`, or `AREA_ABOVE_MAX`. The implementation initializes `AREA_UNKNOWN`, but no fully validated row retains that fallback because valid geometry requires known area and invalid geometry overrides it. Both subsets preserve source-relative order and original index labels. The configured minimum/maximum are inclusive policy thresholds, not geometry measurements; no ranking occurs.
...
`filter_parcels_by_shape` reconstructs and revalidates the exact immutable `ShapeScreeningConfig` and validates its input metrics before either branch. When disabled, retained is an unchanged copy of every input row and rejected is an empty same-schema copy; no policy columns are added and the generated-column collision gate is not reached. When enabled, it rejects pre-existing generated policy/reason columns and applies configured thresholds to already computed metrics. Both outputs append `shape_policy_version`, `shape_policy_min_width_m`, and `shape_policy_max_ratio`; rejected rows also append `shape_rejection_reason`, with precedence `SHAPE_ERROR` before `WIDTH_BELOW_MIN` before `RATIO_ABOVE_MAX`. It preserves each subset's input order/index and does not recalculate shape metrics or infer planning/access/grid/environment meaning.

`profile_shape_distribution` invokes the shared parcel contract, validates physical metric domains and the length/width ratio, requires at least one valid shape, and excludes error rows from percentiles/buckets. Scenario percentages use valid shapes as the denominator. Its frozen diagnostic dataclass still contains mutable dictionaries/lists; it is not a deeply immutable policy/configuration object. Internal median/extreme ranking selects illustrative records only, not a suitability score or a retained/rejected parcel decision.
