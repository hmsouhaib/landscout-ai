...
The core type-contract regressions distinguish three physical layers: exact canonical uppercase `gpkg_geometry_columns.geometry_type_name`, the exact SQL declared type returned for that geometry column by `PRAGMA table_info`, and the actual embedded ISO WKB root. They reject unsupported/noncanonical metadata, SQL mismatches including case/subclass/coercion attempts, and unassignable roots. An independent 8-declaration × 7-root matrix exercises genuine ISO WKB: 17 accepted pairs and 39 rejected pairs. Besides `GEOMETRY`, `GEOMETRYCOLLECTION` is a supported supertype for GeometryCollection and the three Multi* roots, not bare Point/LineString/Polygon. Physical collection-declared fixtures require unchanged actual Multi* names after public build and independent validation, while exact SQL/declaration equality remains mandatory. Intrinsic rehash tests cover individual and mixed compatible domains and reject an inserted bare Polygon. EMPTY XY Multi* and genuine non-empty M/ZM Multi* controls verify exact dimensions and ordinates without adding measured-empty multi-collection capabilities; existing POINT M/ZM/EMPTY and dimension-loss regressions remain. Header-envelope regressions prove code/byte order/framing/length, not numerical agreement of envelope doubles with every WKB coordinate; observed bounds are independently calculated and compared with catalog bounds.
...
The geometry-only real EP verification is an explicitly controlled offline check after synthetic cases pass. It blocks network, Pyogrio/GeoPandas feature readers, repairs, and reprojection; it reads no environmental attribute. The complete per-layer geometry evidence and exact toolchain/hash results are historical validation evidence in `docs/DEV_LOG.md`, not inferred environmental meaning. This geometry-only instrumentation is distinct from the integrated evidence-bundle check, which must exercise the established attribute-only reader as well.

The geometry suite contains 404 collected cases (327 retained + 77 collection-subtype regressions); the four existing INPN source/catalog/attribute/geometry suites contain 803 (399 + 404). Completed execution results are recorded in `docs/DEV_LOG.md`. One all-NULL mandatory Z/M metadata fixture explicitly expects eight Pyogrio measured-type metadata warnings. A separate collection-declaration/SQL-type mismatch negative explicitly expects two catalog and four build RuntimeWarnings before the exact SQL binding fails. These warnings are asserted, not hidden; real geometry rows still bypass Pyogrio entirely.

### INPN source-bound attribute/geometry alignment

Evidence-bundle tests use tiny real ZIP/GeoPackage fixtures and the existing catalog and both physical profile builders/validators. Positive source-complete controls preserve the approved attribute-only `pyogrio.read_dataframe(..., read_geometry=False)` options and geometry-only SQLite FID/BLOB projection; no new combined reader is authorized. Pure structural-helper tests deliberately construct inconsistent profile evidence and are labeled as structural tests, not mocked end-to-end proof.

The contract covers exact public/nested types, source/archive/catalog and repeated package identity, the full `(relative_path, layer_name)` key, complete canonical layer order, every feature/FID count and extrema relationship, and complete FID-sequence hash equality. In particular, equal-count/equal-extrema `[1, 2, 4]` and `[1, 3, 4]` must fail alignment. Sparse, negative, empty, differently read-order FIDs, and M/ZM evidence retain their existing profile meaning. Coordinated upstream/alignment/bundle rehashes must not bypass physical rebuild; source mutation, cache-root/cache-hit portability, immutable nested records, no direct new reader, public exports, and exact public-validator call counts are separate checks.

The bundle suite contains 25 top-level test functions expanding to 141 cases, in addition to the unchanged 803-case four-file INPN baseline. All ten scalar-type cases require the precise `noncanonical runtime type` error so a stale hash cannot mask a missing type guard. Same-class forged configuration must fail reconstruction before physical validation. A coordinated catalog-field/profile/bundle rehash must reach and fail the physical catalog authority rather than rely only on cheap alignment.

The integrated offline EP verification intentionally runs both established reader paths. Instrumentation distinguishes initial upstream builds, bundle builds/repeats, and independent bundle validation, with actual metadata/attribute/SQLite read counts. DNS, HTTP, downloads, geometry-capable Pyogrio reads, repairs, reprojection, and parcel work remain prohibited; the approved attribute-only reader is allowed. Per-layer attribute and geometry FID hashes must be reported equal, without dropping layers or substituting a business identifier. Complete observed evidence belongs in `docs/DEV_LOG.md`; matching FIDs do not resolve the `sig_tadl` CRS/coordinate observation or infer environmental semantics.
