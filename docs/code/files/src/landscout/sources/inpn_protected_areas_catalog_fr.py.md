...
- Domain: official source physical metadata authority
- Responsibility: Builds and independently validates a deterministic source-bound metadata-only catalog of every verified INPN EP GeoPackage, OGR layer, and ordered field.
- Source SHA256: `91224c097f405da47a6fd5be0672cac7f17efd9b682f589ec84d304ac409d33d`

## 1. Purpose and architectural position

Builds and independently validates a deterministic source-bound metadata-only catalog of every verified INPN EP GeoPackage, OGR layer, and ordered field.

## 2. STEP 7F.1B.1 contract

Frozen field, layer, package, and complete-catalog dataclasses preserve exact source/archive lineage and physical ordering. `build_inpn_protected_areas_catalog` consumes only the fresh extraction returned by `validate_inpn_protected_areas_extraction`, uses `pyogrio.list_layers` and `pyogrio.read_info` with forced counts/bounds, verifies package bytes before and after inspection, and revalidates the full extraction after inspection. Schema-1 canonical JSON excludes machine paths, timestamps, cache state, Python representations, and object identity. `validate_inpn_protected_areas_catalog` validates intrinsic domains/source locks, independently rebuilds, and exact-compares every value.
