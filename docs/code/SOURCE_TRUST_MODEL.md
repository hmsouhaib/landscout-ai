...
- Extraction owns one canonical Windows-compatible relative-path grammar, and catalog/profile intrinsic validation delegates to it before accepting package identity. Unsafe reserved, forbidden/control, component-whitespace, trailing-dot/space, traversal/absolute/driven/backslash, and NFKC-hazard forms therefore fail identically at all three layers; validation rejects whitespace rather than trimming it, and valid nested `.gpkg`/`.GPKG` spellings are retained exactly. Catalog/profile boundaries translate failures to their public controlled errors with chained causes.
...
- The current chain stops at factual EP attribute values. EP is distinct from the separate Natura 2000 and ZNIEFF reference archives. It does not materialize EP geometry, interpret categories, or perform parcel analysis.
