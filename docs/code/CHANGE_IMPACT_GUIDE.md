...
Review exact extraction/config/catalog runtime types, fresh catalog reconstruction, the single extraction-owned Windows-compatible canonical relative-package-path grammar, exact preservation of accepted `relative_path: str`, rejection rather than trimming of whitespace, catalog/profile error translation with chained causes, package path/position bijection, lexical contiguous package/layer grouping, repeated package-evidence equality, exact/NFKC/casefold identity uniqueness, contiguous layer/field positions, FID extrema/inclusive capacity, package-byte helper ownership, the shared narrow Pyogrio warning context, exact `read_dataframe` options, Pandas-only frame/type/column/order/count rules, integer FID preservation and sorting, null detection, the closed text/Boolean/integer/float-hex/Base64 scalar representation, unsupported composite/temporal/geometry failures, complete distinct domains/frequencies, FID/column/row/profile canonical JSON hashes, deterministic empty-component hashes, aggregate closure, the cheap fresh-catalog preflight before attribute reads, final extraction/catalog postconditions, and independent physical rebuild. Column hashes bind only ordered FID-addressed canonical cells; field identity/dtypes are separate complete-profile facts. Intrinsic validation checks non-empty component-SHA syntax but physical validation reconstructs those hashes. Profiles retain portable relative-path lineage but no absolute filesystem `Path`. EP remains distinct from Natura 2000 and ZNIEFF; category meaning, geometry loading, parcel relations, exclusions, and scores remain outside this layer.

## Changing the INPN geometry-quality profile

Review the unchanged archive/extraction/package-byte authority, fresh catalog reconstruction, exact immutable bytes into `sqlite3.Connection.deserialize`, connection closure, query-only/trusted-schema settings, no extensions/ATTACH/writes, exact feature-table and geometry-column metadata, INTEGER PRIMARY KEY rowid alias, unsupported views, quoted identifiers, bound values, and the strict FID-plus-BLOB-only projection. Pyogrio 0.13.0 is intentionally excluded from geometry-row materialization because it drops M; it remains the approved metadata reader. Review Standard GeoPackageBinary header byte order, magic/version/reserved/extended/envelope flags, signed SRS ID, independent embedded-WKB byte order, empty-state agreement, EPSG/SRS agreement, and mandatory/prohibited/optional Z/M declarations. Keep real-parser XY/XYZ/XYM/XYZM and all four EMPTY variants permanent; never make a failing measured test pass by removing M.

Review canonical FIDs, null/empty separation, coordinate finiteness including Z/M, type/dimension/validity-reason domains, invalid-topology retention, exact bounds relations, immutable output fields, raw full-BLOB versus parser-derived WKB hash separation, explicit encoding options, toolchain identity, complete canonical JSON, intrinsic structure/hash validation, cheap fresh-catalog preflight before geometry-row reads, source-complete rebuild, and final extraction/catalog equality. The intrinsic validator cannot reconstruct non-empty raw/parser hashes without rows. Tests must instrument zero attribute projections and zero Pyogrio/GeoPandas feature reads, transient/persistent path swaps, coordinated hash forgery, and offline portable-repeat evidence. Geometry normalization/repair/reprojection, environmental semantics, and parcel analysis remain unimplemented. Do not change the approved catalog or attribute payload/schema/hash as a side effect.

## Changing the INPN evidence bundle

Review the exact catalog/attribute/geometry public model types and schemas, existing intrinsic validators, mandatory public physical profile validators, source/archive/catalog identity, complete catalog-ordered inventory under `(relative_path, layer_name)`, repeated package evidence, exact layer positions/counts, both FID counts/extrema/sequence hashes, and the final extraction postcondition. Structural alignment is private and cannot substitute for either physical profile validator. Reuse the existing attribute-only Pyogrio and geometry-only SQLite readers through their public validators; do not add a third reader or blanket-ban the attribute-only call when instrumenting the integrated chain.

Review ten-field immutable alignment records, retained immutable upstream models, canonical compact bundle payload, schema/hash closure, chained lower-layer errors, and independent expected-bundle reconstruction. The new hash binds upstream schemas/complete hashes and ordered alignments, not duplicate expanded domains; geometry toolchain dependence is inherited. Keep upstream catalog/attribute/geometry schemas, payloads, and hashes unchanged unless a separately justified migration is approved. Test equal count/extrema with differing FID sets, sparse/negative/empty identities, same layer names in distinct packages, reordered/missing/extra layers, coordinated upstream/alignment rehashes, physical mutation, portable cache-root/cache-hit equality, deep immutability, and real source-complete positive controls. Clearly label pure structural-helper fixtures rather than presenting mocked physical validators as end-to-end proof. Alignment adds no new raw reader or joined per-feature representation; the approved validators still materialize their own source rows internally. It adds no protected-area meaning, CRS correction, parcel overlay, or scoring.

## Future INPN semantic boundary

Start with the [French EP 07/2026 reference](../reference/inpn_ep_2026_07_semantic_reference.md) and [paired research JSON](../reference/inpn_ep_2026_07_semantic_reference.json). They are documentation-only `RESEARCH_DRAFT_NOT_RUNTIME_POLICY`, not a loader/configuration or approved executable mapping. Preserve exact composite value keys, all marginal frequencies/nulls and source column hashes. Marginal domains cannot establish cross-field co-occurrence, delimiter grammar, legal restrictions or BESS feasibility.

Before any separately approved executable mapping, resolve producer export-to-standard edition/field/value provenance, consultation-versus-adoption conflicts, local jurisdiction/version limits and explicit unknowns. Review observed-value coverage separately from confirmed snapshot meaning. Do not promote `OFFICIAL_CONTEXT_ONLY` or `UNRESOLVED` entries into runtime rules, and do not alter the approved evidence readers or bundle merely to interpret this draft.
