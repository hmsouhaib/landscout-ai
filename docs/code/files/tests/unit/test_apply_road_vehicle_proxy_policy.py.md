...
- Responsibility: Exercises application classification and its mocked normalization boundary; it does not independently validate a physical IGN source.
...
Exercises application classification and its mocked normalization boundary; it does not independently validate a physical IGN source.

### Audited fixture and assertion scope

The 28 test definitions use five local helpers and no local Pytest fixtures. `_base_row` supplies normalized factual columns; `_roads` constructs an in-memory EPSG:2154 GeoDataFrame; `_source` constructs an exact `IgnBdTopoRoadData` whose extraction and summary are deliberately absent. `_apply` replaces `normalize_ign_roads` with an already-normalized result, while retaining the real checked-in policy loader and classifier. Consequently these tests exercise scalar parsing, all sixteen business rules, eight precedence conflicts, deterministic traces, unknown evidence, lineage, copying and controlled application errors, not physical GPKG reconstruction. Physical reconstruction is covered by the normalization/source suites.

The normalization-count test asserts that the mock was called once, not its exact arguments. Normalization failure and generated-column collision tests assert that policy loading is not reached. Width boundaries include missing/NaN, exactly 2.9, just below 2.9, nonpositive, infinite, string and Boolean inputs; strings are never silently accepted as numeric widths. Frozen-result coverage tests field reassignment on the result wrapper, not deep immutability of its retained GeoDataFrame.

Two geometry tests have narrower evidence than their names suggest. The `INVALID` parameter injects that factual status alongside a self-crossing LineString; it tests dispatch on the supplied status, not Shapely invalidity detection. `test_valid_geometry_status_with_unsupported_geometry_is_not_repaired` constructs a local Polygon frame but does not pass that frame to the application: a patched normalizer raises first, and the test then checks the unused frame. Its demonstrated contract is normalization-error translation plus no mutation of that local object, not physical Polygon rejection. The complete source below retains these distinctions for future test maintenance.
...
**Purpose:** Checks translation of a mocked normalization failure and that a separately constructed, unused Polygon frame remains unchanged. The Polygon frame is not supplied to the application, so this assertion does not establish actual source geometry rejection or repair behavior.
...
|---|---|---|---:|---|
| `test_public_api_exports_only_stable_application_symbols` | none | none | 4 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_wrong_source_type_has_controlled_error` | none | pytest.raises(IgnRoadVehicleProxyApplicationError) | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_wrong_source_config_type_has_controlled_error` | none | pytest.raises(IgnRoadVehicleProxyApplicationError) | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_malformed_policy_path_has_controlled_error` | none | pytest.raises(IgnRoadVehicleProxyApplicationError) | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_source_complete_normalization_is_invoked_exactly_once` | none | none | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_normalization_failure_stops_policy_loading` | none | pytest.raises(IgnRoadVehicleProxyApplicationError) | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_generated_policy_column_collision_fails_before_policy_loading` | none | pytest.raises(IgnRoadVehicleProxyApplicationError, match="collide.*generated") | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_normalized_facts_rows_index_crs_and_geometry_are_preserved` | none | none | 7 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_source_object_is_not_mutated` | none | none | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_non_valid_geometry_uses_technical_gate` | pytest.mark.parametrize(<br>    ("status", "geometry"),<br>    [<br>        ("NULL", None),<br>        ("EMPTY", LineString()),<br>        ("INVALID", LineString([(0, 0), (1, 1), (0, 1), (1, 0)])),<br>    ],<br>) | none | 3 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_unknown_geometry_status_is_rejected` | none | pytest.raises(IgnRoadVehicleProxyApplicationError) | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_each_policy_rule_selects_approved_outcome` | pytest.mark.parametrize(<br>    ("overrides", "rule", "expected_class"),<br>    [<br>        ({"fictitious_raw": True}, "FICTITIOUS_GEOMETRY", "NOT_DISTANCE_PROXY"),<br>        (<br>            {"asset_status_raw": "En projet"},<br>            "PROJECT_GEOMETRY_NOT_SIGNIFICANT",<br>            "NOT_DISTANCE_PROXY",<br>        ),<br>        (<br>            {"asset_status_raw": "En construction"},<br>            "NOT_IN_SERVICE",<br>            "NOT_GENERAL_VEHICLE_PROXY",<br>        ),<br>        (<br>            {"light_vehicle_access_raw": "Physiquement impossible"},<br>            "PHYSICALLY_IMPOSSIBLE",<br>            "NOT_GENERAL_VEHICLE_PROXY",<br>        ),<br>        (<br>            {"nature_raw": "Escalier"},<br>            "NON_GENERAL_VEHICLE_NATURE",<br>            "NOT_GENERAL_VEHICLE_PROXY",<br>        ),<br>        (<br>            {"light_vehicle_access_raw": "Restreint aux ayants droit"},<br>            "RIGHTS_RESTRICTED",<br>            "RESTRICTED_REVIEW",<br>        ),<br>        ({"private_raw": 1.0}, "PRIVATE_ROAD", "RESTRICTED_REVIEW"),<br>        (<br>            {"closure_period_raw": "Fermeture hivernale"},<br>            "TEMPORAL_CLOSURE",<br>            "RESTRICTED_REVIEW",<br>        ),<br>        (<br>            {"restriction_nature_raw": "Plot amovible"},<br>            "KNOWN_RESTRICTION",<br>            "RESTRICTED_REVIEW",<br>        ),<br>        (<br>            {"restriction_nature_raw": "Nouvelle restriction"},<br>            "OTHER_RECORDED_RESTRICTION",<br>            "RESTRICTED_REVIEW",<br>        ),<br>        (<br>            {"nature_raw": "Bac ou liaison maritime"},<br>            "SPECIAL_NATURE",<br>            "RESTRICTED_REVIEW",<br>        ),<br>        ({"nature_raw": "Chemin"}, "LIMITED_NATURE", "LIMITED_VEHICLE_PROXY"),<br>        ({"importance_raw": "6"}, "IMPORTANCE_6", "LIMITED_VEHICLE_PROXY"),<br>        (<br>            {"carriageway_width_raw": 2.8},<br>            "NARROW_CARRIAGEWAY",<br>            "LIMITED_VEHICLE_PROXY",<br>        ),<br>        ({}, "OPEN_OR_TOLL", "GENERAL_VEHICLE_PROXY"),<br>        ({"nature_raw": "Future"}, "UNKNOWN", "UNKNOWN_REVIEW"),<br>    ],<br>) | none | 2 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_policy_precedence_conflicts_select_first_rule` | pytest.mark.parametrize(<br>    ("overrides", "rule"),<br>    [<br>        ({"fictitious_raw": True, "private_raw": 1.0}, "FICTITIOUS_GEOMETRY"),<br>        (<br>            {<br>                "asset_status_raw": "En projet",<br>                "light_vehicle_access_raw": "Physiquement impossible",<br>            },<br>            "PROJECT_GEOMETRY_NOT_SIGNIFICANT",<br>        ),<br>        (<br>            {<br>                "light_vehicle_access_raw": "Physiquement impossible",<br>                "private_raw": 1.0,<br>            },<br>            "PHYSICALLY_IMPOSSIBLE",<br>        ),<br>        ({"private_raw": 1.0, "carriageway_width_raw": 2.5}, "PRIVATE_ROAD"),<br>        (<br>            {"closure_period_raw": "Hiver", "nature_raw": "Chemin"},<br>            "TEMPORAL_CLOSURE",<br>        ),<br>        (<br>            {<br>                "restriction_nature_raw": "Plot amovible",<br>                "nature_raw": "Bac ou liaison maritime",<br>            },<br>            "KNOWN_RESTRICTION",<br>        ),<br>        ({"nature_raw": "Chemin", "importance_raw": "6"}, "LIMITED_NATURE"),<br>        (<br>            {"importance_raw": "6", "carriageway_width_raw": 2.5},<br>            "IMPORTANCE_6",<br>        ),<br>    ],<br>) | none | 1 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_boolean_like_source_values_are_parsed_without_coercion` | pytest.mark.parametrize(<br>    ("field", "value", "expected_rule"),<br>    [<br>        ("fictitious_raw", False, "OPEN_OR_TOLL"),<br>        ("fictitious_raw", np.bool_(True), "FICTITIOUS_GEOMETRY"),<br>        ("fictitious_raw", None, "UNKNOWN"),<br>        ("fictitious_raw", "true", "UNKNOWN"),<br>        ("private_raw", False, "OPEN_OR_TOLL"),<br>        ("private_raw", True, "PRIVATE_ROAD"),<br>        ("private_raw", 0, "OPEN_OR_TOLL"),<br>        ("private_raw", 1, "PRIVATE_ROAD"),<br>        ("private_raw", 0.0, "OPEN_OR_TOLL"),<br>        ("private_raw", 1.0, "PRIVATE_ROAD"),<br>        ("private_raw", None, "UNKNOWN"),<br>        ("private_raw", 2, "UNKNOWN"),<br>        ("private_raw", "1", "UNKNOWN"),<br>    ],<br>) | none | 1 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_unknown_critical_vocabulary_never_uses_general_fallback` | pytest.mark.parametrize(<br>    ("field", "value"),<br>    [<br>        ("asset_status_raw", "Future"),<br>        ("nature_raw", "Future"),<br>        ("light_vehicle_access_raw", "Future"),<br>        ("importance_raw", "7"),<br>        ("importance_raw", 6),<br>    ],<br>) | none | 3 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_width_contract` | pytest.mark.parametrize(<br>    ("value", "expected_rule"),<br>    [<br>        (None, "OPEN_OR_TOLL"),<br>        (float("nan"), "OPEN_OR_TOLL"),<br>        (2.9, "OPEN_OR_TOLL"),<br>        (2.899999, "NARROW_CARRIAGEWAY"),<br>        (0.0, "UNKNOWN"),<br>        (-1.0, "UNKNOWN"),<br>        (float("inf"), "UNKNOWN"),<br>        ("2.8", "UNKNOWN"),<br>        (True, "UNKNOWN"),<br>    ],<br>) | none | 1 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_optional_restriction_source_contract` | pytest.mark.parametrize(<br>    ("field", "value", "expected_rule"),<br>    [<br>        ("closure_period_raw", None, "OPEN_OR_TOLL"),<br>        ("closure_period_raw", "Hiver", "TEMPORAL_CLOSURE"),<br>        ("closure_period_raw", " ", "UNKNOWN"),<br>        ("closure_period_raw", 1, "UNKNOWN"),<br>        ("restriction_nature_raw", None, "OPEN_OR_TOLL"),<br>        ("restriction_nature_raw", "Plot amovible", "KNOWN_RESTRICTION"),<br>        (<br>            "restriction_nature_raw",<br>            "Restriction nouvelle",<br>            "OTHER_RECORDED_RESTRICTION",<br>        ),<br>        ("restriction_nature_raw", "", "UNKNOWN"),<br>        ("restriction_nature_raw", 1, "UNKNOWN"),<br>    ],<br>) | none | 1 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_every_configured_known_restriction_is_applied` | none | none | 1 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_general_fallback_requires_complete_positive_evidence_and_tracks_toll` | none | none | 4 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_open_access_does_not_hide_unresolved_evidence` | pytest.mark.parametrize(<br>    "overrides",<br>    [<br>        {"nature_raw": "Future"},<br>        {"private_raw": None},<br>        {"importance_raw": "7"},<br>        {"carriageway_width_raw": "wide"},<br>    ],<br>) | none | 1 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_trace_is_complete_unique_and_in_policy_order` | none | none | 2 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_known_higher_rule_remains_primary_while_unknown_is_traced` | none | none | 3 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_unknown_fields_trace_is_fixed_and_deterministic` | none | none | 2 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_policy_lineage_is_exact_on_every_row` | none | none | 7 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_result_is_frozen_and_contains_no_unsafe_claim_vocabulary` | none | pytest.raises(FrozenInstanceError) | 1 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_valid_geometry_status_with_unsupported_geometry_is_not_repaired` | none | pytest.raises(IgnRoadVehicleProxyApplicationError) | 1 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_policy_path_must_be_path_or_none` | none | pytest.raises(IgnRoadVehicleProxyApplicationError) | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
| `test_source_config_is_exact_pydantic_type` | none | pytest.raises(IgnRoadVehicleProxyApplicationError) | 0 | See the callable's section 6 setup/assertions and audited test-scope notes; section 11 retains the complete source. |
